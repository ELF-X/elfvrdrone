...
static int32_t spi_id;
static int32_t slave_num;
 ...
 */
static int32_t PIOS_MS5611_Validate()
{
	if (spi_id == 0)
		return -3;
	return 0;
...
int32_t PIOS_MS5611_SPI_Init(uint32_t spiid, uint32_t slavenum, const struct pios_ms5611_cfg *cfg)
{
    spi_id = spiid;
	slave_num = slavenum;
...
static int32_t PIOS_MS5611_Read(uint8_t address, uint8_t *buffer, uint8_t len)
{
	...
	if (PIOS_SPI_TransferByte(spi_id, address) < 0) {
		rc = -3;
		goto out;
	}

	if (PIOS_SPI_TransferBlock(spi_id, NULL, buffer, len, NULL) < 0) {
		rc = -3;
		goto out;
	}
...
static int32_t PIOS_MS5611_WriteCommand(uint8_t command)
{
	...
	int32_t rc;
	if (PIOS_SPI_TransferByte(spi_id, command) < 0) {
		rc = -3;
		goto out;
	}
...
static int32_t PIOS_MS5611_ReleaseBus(void)
{
	...
	PIOS_SPI_RC_PinSet(spi_id, slave_num, 1);
...
static int32_t PIOS_MS5611_ClaimBus(void)
{
	...
	PIOS_SPI_RC_PinSet(spi_id, slave_num, 0);
